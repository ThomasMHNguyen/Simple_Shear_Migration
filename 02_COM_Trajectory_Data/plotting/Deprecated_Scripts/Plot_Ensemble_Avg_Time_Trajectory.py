# -*- coding: utf-8 -*-
"""
FILE NAME:      Plot_Ensemble_Avg_Time_Trajectory.py
...
OUTPUT
FILES(S):       1) .PNG file that shows the ensemble average trajectory.
                2) .PDF file that shows the ensemble average trajectory.
                3) .EPS file that shows the ensemble average trajectory.
...
def plot_avg_com_time_displacement_data(ensemble_data_df,output_dir):
    """
    ...
    for group in exp_groups.groups.keys():
        ...
        if len(str_mu_bar_v1[0].split('.')[1]) == 1:
            if str_mu_bar_v1[0].split('.')[1] == 0:
                pref = '{0}p{1}{2}'.format(str_mu_bar_v1[0].split('.')[0],'0',
                                           str_mu_bar_v1[0].split('.')[1])
            else:
                pref = '{0}p{1}{2}'.format(str_mu_bar_v1[0].split('.')[0],
                                   str_mu_bar_v1[0].split('.')[1],'0')
        else:
        ...
        if flow_type == 'Shear':
            abr_flow_txt = 'SHR'
            filename_prefix = 'YCOM_{}_ST_{}_MB_{}_H_{}'.format(steric_use,
                                                            abr_flow_txt,
                                                            mu_bar_text,
                                                            channel_h_text)
            if steric_use == 'Yes':
            ...
            abr_flow_txt = 'POI'
            filename_prefix = 'YCOM_{}_ST_{}_MB_{}_H_{}_UC_{}'.format(steric_use,
                                                                 abr_flow_txt,
                                                                 mu_bar_text,
                                                                 channel_h_text,
                                                                 U_c_text)
            ...
            abr_flow_txt = 'KMG'
            filename_prefix = 'YCOM_{}_ST_{}_MB_{}_H_{}_FR_{}_PH_{}'.format(steric_use,
                                                                       abr_flow_txt,
                                                                       mu_bar_text,
                                                                       channel_h_text,
                                                                       int(k_freq),
        ...
        elif group_df['Brownian Time'].max() == 8e-2:
            axes.set_xlim(-8e-6,8.2e-2)
            axes.set_xticks(np.linspace(0,8e-2,5))
...
def plot_avg_com_sq_time_displacement_data(ensemble_data_df,output_dir):
    """
    This function plots the ensemble average center of mass trajectory of a 
    filament based on a particular rigidity profile, 
    flow strength value, channel height, starting vertical displacement, 
    ...
    for group in exp_groups.groups.keys():
        ...
        ##### Center of Mass #####
        fig,axes = plt.subplots(figsize = (7,7))
        sns.lineplot(y = 'SQ Center of Mass-y',x = 'Brownian Time',hue = 'Starting Vertical Displacement',
                                palette = 'tab20',data = group_df,linewidth = 2.5,
                                legend = False,ax = axes)
        axes.ticklabel_format(axis="x", style="sci", scilimits=(-2,-2))
        axes = plt.gca()
...
def plot_avg_tc_time_displacement_data(ensemble_data_df,output_dir):
    """
...
if __name__ == '__main__':
    ...
    plot_avg_com_time_displacement_data(ensemble_data_df,args.output_directory)
    plot_avg_tc_time_displacement_data(ensemble_data_df,args.output_directory)
