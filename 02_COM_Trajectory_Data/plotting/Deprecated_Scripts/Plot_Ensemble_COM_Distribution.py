...
def plot_com_distribution_new(ensemble_data_df,time_vals,time_vals_text,output_dir):
    ...
    for group in exp_groups.groups.keys():
        ...
        if flow_type == 'Poiseuille' or flow_type == 'Shear':
            ax0.set_xlabel(r"$U_{x} (y)$",
                                fontsize = 11,labelpad = 5)                  
            ax0.set_ylabel(r"$y$",fontsize = 13,labelpad = 5)
            ax.set_ylim(-0.05,np.round(1.1*channel_h,2))
            ax0.set_yticks(np.linspace(0,0.5,6))
            if flow_type == 'Poiseuille':
                ax0.set_xlim(-0.1,1.6)
                ax0.set_xticks(np.linspace(-0,1.5,6))
                ax0.hlines(y = channel_h,xmin =-0.1,xmax = 1.6,
                ...
                ax0.hlines(y = channel_h,xmin =-0.05,xmax = 0.6,
                         color = 'black',linestyle = 'dashed',linewidth = 1.2,alpha = 0.7)
                [l.set_visible(False) for (i,l) in enumerate(ax.xaxis.get_ticklabels()) if (i) % 2 != 0]
        ...
        ### Formatting on plots ###
            for n,ax in enumerate(all_subplot_grid_axes):
                    ...
                    ax.set_title(r"${0}$".format(time_vals_text[n - 1]),
                                        fontsize = 11,pad = 5)
        ...
        ### Formatting on plots ###
            for n,ax in enumerate(axes):
                    if n > 0:
                        ...
                        ## Format time values
                        if n  == 2:
                            ax.set_title(r"${0:.3f}$".format(time_vals[n - 1]),
                                                fontsize = 11,pad = 5)
                        else:
                            ax.set_title(r"$ {0:.2f}$".format(time_vals[n - 1]),
                                                fontsize = 11,pad = 5)
