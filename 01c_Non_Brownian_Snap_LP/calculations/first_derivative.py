...
def first_derivative(base_array,deriv_size,axis,ar_size,dim):
    ...
    if dim == 1:
        ...
        dds_base[0] = (0.5/deriv_size)*(-3*base_array[0] + 4*base_array[1] - 1*base_array[2])
        dds_base[ar_size-1] = (0.5/deriv_size)*(3*base_array[ar_size-1] - 4*base_array[ar_size-2] + 1*base_array[ar_size-3])
        dds_base[1:ar_size-1] = (0.5/deriv_size)*(-base_array[0:ar_size-2] + base_array[2:ar_size])
    if dim == 2:
        dds_base = np.zeros((base_array.shape),dtype = float)
        if axis == 0:
            dds_base[0,:] = (0.5/deriv_size)*(-3*base_array[0,:] + 4*base_array[1,:] - 1*base_array[2,:])
