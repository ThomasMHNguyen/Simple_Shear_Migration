...
DESCRIPTION:    This script will read in the filament position data and simulation
                parameters performing an downward flip in Poiseuille flow. 
...
1)              .PDF/.SVG file that shows center of mass curves as a function of
                time for an downward flip in Poiseuille flow. A curve
...
def plot_center_of_mass_xs_down_poi(time_data,position_data,center_of_mass_data,xs_data,\
    ...
    ax1 = axes.twinx()
    # axins_0 = inset_axes(axes, width="70%", height="70%",loc = 'upper left',bbox_to_anchor = (55,250,200,200))
    ...
    # axins_0.plot(position_data[:,0,430],position_data[:,1,430],color = 'black',linewidth = 2)
    # axins_1.plot(position_data[:,0,550],position_data[:,1,550],color = 'black',linewidth = 2)
    # axins_2.plot(position_data[:,0,875],position_data[:,1,875],color = 'black',linewidth = 2)
    
    # axins_0.annotate(text = "",xy = (0.10,0.12),
    #                  xytext = (0.3,0.25),arrowprops=dict(arrowstyle='->', connectionstyle='arc3,rad=0.3', 
    #                             color='red',linewidth = 2,shrinkA=0.4, shrinkB=0.4))
    # axins_1.annotate(text = "",xy = (-0.05,0.23),
    #                   xytext = (0.25,0.11),arrowprops=dict(arrowstyle='->', connectionstyle='arc3,rad=0.0', 
    #                             color='red',linewidth = 2,shrinkA=0.4, shrinkB=0.4))
    # axins_1.annotate(text = "",xy = (0.16,-0.15),
    #                   xytext = (-0.15,-0.08),arrowprops=dict(arrowstyle='->', connectionstyle='arc3,rad=0.0', 
    #                             color='red',linewidth = 2,shrinkA=0.4, shrinkB=0.4))
    ...
    ### Format axes ###
    axes.set_ylabel(r"$\vert \Delta y^{\text{com}}\vert $",fontsize = 17,labelpad = 5)
    ...
    ax1.set_ylabel(r"$\text{max}\left(x_{s\vert y}\right) $",fontsize = 17,labelpad = 5)
    ...
    axes.text(x = 2,y = 0.042,s = r"\textbf{I}",size = 18)
    axes.text(x = 7.5,y = 0.042,s = r"\textbf{II}",size = 18)
    axes.text(x = 16,y = 0.042,s = r"\textbf{III}",size = 18)
    
    fig.suptitle("Poiseuille-Down Flip",size = 17)
