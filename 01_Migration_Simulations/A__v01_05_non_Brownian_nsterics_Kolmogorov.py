...
19Sep22         22) Code reformatted for non-Brownian cases. Fixed issue with
                code not running with steric check turned off. 
...
def fluid_velo(sim_c,x_vec):
    ...
    u0[:,0] = np.sin(k_flow_factor*k_flow_phase*x_vec[:,1])
...
class run_parameters():
    ...
    def __init__(self,sim_params,end_time,start_time):
        ...
        self.parameter_df.loc['Flow Type','Value'] = 'Kolmogorov'
        self.parameter_df.loc['Sterics Use','Value'] = 'No'
...
def run_simulation(target_dir,rigid_type,mu_bar,rep):
    ...
    sim_c = Constants(end_dir = target_dir,rigid_func = rigid_type,
                                ...
                                top = 0, bottom = 1,U_centerline = 0.00,k_flow_phase = np.pi,k_flow_factor = 1,
                                gap_criteria = 0.02,force_power = 12,channel_height = 0.50,
                                vert_displace = 0.25,N = 101,reg_dt = 0.005,adpt_dt = 0.0005,
                                end_time = 40,save_time = 0.01)
    vert_displ_str = '{:.2f}'.format(sim_c.vertical_displacement).replace('.','p')
    U_phase_str = '{:.2f}'.format(sim_c.kflow_phase).replace('.','p')
    U_val_str = '{:.2f}'.format(sim_c.kflow_freq).replace('.','p')
    dir_name = 'VD_{}/{}_UF_{}_P_{}/MB_{}/R_{}'.format(vert_displ_str,
                                                                sim_c.rigidity_suffix,U_val_str,
                                                                U_phase_str,int(sim_c.mu_bar),rep)
...
def Main():
    ...
    parser.add_argument("replicate_value",
                        ...
                        type = int)
    args = parser.parse_args(['./02_Actual_Results/Non_Brownian_Kolmogorov_Down/Vert_Displ_0p25/' ,'K_constant', '100000','1']) #Uncommment this line when debugging
...
snapshot_time_vals = np.array([4,8,12,20,40])
