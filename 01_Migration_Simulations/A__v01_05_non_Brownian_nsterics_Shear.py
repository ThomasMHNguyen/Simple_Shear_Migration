...
def Main():
    ...
    parser.add_argument("replicate_value",
                        ...
                        type = int)
    args = parser.parse_args(['./02_Actual_Results/NB_Shear_J/MB_1p00e5/Up_0p30/' ,'K_constant', '100000','1']) #Uncommment this line when debugging
...
plt.figure()
plt.plot(time_vals[:np.where(time_vals == 50)[0][0]+1],np.abs(com_data - 0.30)[:np.where(time_vals == 50)[0][0]+1])
plt.show()
...
norm_time_plot_vals = np.round(np.arange(0,0.75,0.01),2)
