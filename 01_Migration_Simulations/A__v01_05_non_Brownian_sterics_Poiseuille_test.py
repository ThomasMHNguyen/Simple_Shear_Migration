...
def construct_lhs_matrix(sim_c,adj_xs):
    ...
    for i in range(2,N-2):
        ...
        for idx,val in enumerate(block_pos): #Iterate through each index corresponding to the sub-block from the center sub-block
            lhs_matrix[3*i:3*i+3,3*i+val:3*i+val+3] += prefactors[idx]*dyad_p
...
def run_simulation(target_dir,rigid_type,mu_bar,rep):
    ...
    sim_c = Constants(end_dir = target_dir,rigid_func = rigid_type,
                                ...
                                vert_displace = 0.00,N = 101,reg_dt = 0.005,adpt_dt = 0.0005,
...
def Main():
    ...
    parser.add_argument("replicate_value",
                        ...
                        type = int)
    args = parser.parse_args(['./02_Actual_Results/Non_Brownian_Poiseuille_Down_0p00/' ,'K_constant', '50000','1']) #Uncommment this line when debugging
...
axes.plot(prev_info[0][:,0],prev_info[0][:,1],'r')
axes.plot(curr_info[0][:,0],curr_info[0][:,1],'b')
