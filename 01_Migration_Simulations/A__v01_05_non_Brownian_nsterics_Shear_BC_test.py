...
class Constants:
    ...
    def __init__(self,end_dir,rigid_func,slender_f,mubar,true_zeta_f,adpt_zeta_f,
                 top, bottom,U_centerline,k_flow_phase,k_flow_factor,
                 gap_criteria,force_power,channel_height,vert_displace,
                 N,reg_dt,adpt_dt,end_time,save_time):
        ...
        # self.start_y += 0.05*self.s**2
        # self.start_y += (1/200)*np.exp(-self.s)**4 #Upward flip
        self.start_y += (-1/800)*np.exp(self.s)**4 #Downward flip
        self.start_z = np.zeros(self.N)
        self.initial_loc = np.column_stack((self.start_x,self.start_y,self.start_z))
        
        # self.initial_loc[0,:] = (1/11)*(48*self.initial_loc[2,:] - 52*self.initial_loc[3,:] + 15*self.initial_loc[4,:])
        # self.initial_loc[1,:] = (1/11)*(28*self.initial_loc[2,:] - 23*self.initial_loc[3,:] + 6*self.initial_loc[4,:])
        # self.initial_loc[self.N-1,:] = (1/11)*(48*self.initial_loc[self.N-3,:] - 52*self.initial_loc[self.N-4,:] + 15*self.initial_loc[self.N-5,:])
        # self.initial_loc[self.N-2,:] = (1/11)*(28*self.initial_loc[self.N-3,:] - 23*self.initial_loc[self.N-4,:] + 6*self.initial_loc[self.N-5,:])
...
class run_parameters():
    def __init__(self,sim_params,end_time,start_time):
        self.parameter_df = pd.DataFrame(index = ['Random Seed','Filament Length','Filament Persistence Length','Filament s start','Filament s end',
                                              'Poiseuille U Centerline','Kolmogorov Phase','Kolmogorov Frequency',
                                              'Channel Upper Height','Channel Lower Height','Vertical Displacement',
        ...
        if sim_params.kflow_phase == np.pi:
            self.parameter_df.loc['Kolmogorov Phase'] = 'Pi'
        else:
...
def eval_time_euler(sim_c):
    ...
    future_xbar[0,:] = (1/11)*(48*future_xbar[2,:] - 52*future_xbar[3,:] + 15*future_xbar[4,:])
    future_xbar[1,:] = (1/11)*(28*future_xbar[2,:] - 23*future_xbar[3,:] + 6*future_xbar[4,:])
    future_xbar[N-1,:] = (1/11)*(48*future_xbar[N-3,:] - 52*future_xbar[N-4,:] + 15*future_xbar[N-5,:])
    future_xbar[N-2,:] = (1/11)*(28*future_xbar[N-3,:] - 23*future_xbar[N-4,:] + 6*future_xbar[N-5,:])
...
def run_simulation(target_dir,rigid_type,mu_bar,rep):
    ...
    sim_c = Constants(end_dir = target_dir,rigid_func = rigid_type,
                                ...
                                top = 0, bottom = 200,U_centerline = 0.00,k_flow_phase = 0,k_flow_factor = 0,
                                gap_criteria = 0.02,force_power = 12,channel_height = 0.50,
                                vert_displace = 0.30,N = 101,reg_dt = 0.005,adpt_dt = 0.0005,
                                end_time = 75,save_time = 0.01)
...
def Main():
    ...
    parser.add_argument("replicate_value",
                        ...
                        type = int)
    args = parser.parse_args(['./02_Actual_Results/NB_Shear_J_Redo/MB_1p00e5/Down_0p30_sup_small/' ,'K_constant', '100000','1']) #Uncommment this line when debugging
...
### Actual perturbation ###
# true_tension_t0 = sim_data.all_data[17][:,0].copy()
# true_tension_t1 = sim_data.all_data[17][:,1].copy()
# true_tension_t2 = sim_data.all_data[17][:,2].copy()

# small_tension_t0 = sim_data.all_data[17][:,0].copy()
# small_tension_t1 = sim_data.all_data[17][:,1].copy()
# small_tension_t2 = sim_data.all_data[17][:,2].copy()

# p_1 = (1/200)*np.exp(4*sim_data.s)
# p_2 = (1/800)*np.exp(4*sim_data.s)
# fig,axes = plt.subplots(ncols = 2,figsize = (10,7))
# axes[0].plot(sim_data.s,p_1,label = "$P_{1}(s) = 200^{-1}e^{4s}$",color = 'red')
# axes[0].plot(sim_data.s,p_2,label = "$P_{2}(s) = 800^{-1}e^{4s}$",color = 'blue')
# axes[1].plot(sim_data.s,true_tension_t0,color = 'red',linestyle = 'solid',label  = "$P_{{1}},t = {0:.2e}$".format(sim_data.all_time_vals[0]))
# axes[1].plot(sim_data.s,true_tension_t1,color = 'red',linestyle = 'dashed',label  = "$P_{{1}},t = {0:.2e}$".format(sim_data.all_time_vals[1]))
# axes[1].plot(sim_data.s,true_tension_t2,color = 'red',linestyle = 'dotted',label  = "$P_{{1}},t = {0:.2e}$".format(sim_data.all_time_vals[2]))
# axes[1].plot(sim_data.s,small_tension_t0,color = 'blue',linestyle = 'solid',label  = "$P_{{2}},t = {0:.2e}$".format(sim_data.all_time_vals[0]))
# axes[1].plot(sim_data.s,small_tension_t1,color = 'blue',linestyle = 'dashed',label  = "$P_{{2}},t = {0:.2e}$".format(sim_data.all_time_vals[1]))
# axes[1].plot(sim_data.s,small_tension_t2,color = 'blue',linestyle = 'dotted',label  = "$P_{{2}},t = {0:.2e}$".format(sim_data.all_time_vals[2]))
# axes[0].set_ylim(-0.001,0.040)
# axes[0].set_yticks(np.linspace(0,0.040,5))
# axes[0].set_ylabel("$P(s)$")
# axes[0].set_xlabel("$s$")
# axes[0].set_title("Perturbation")
...
# plt.savefig(os.path.join(sim_data.output_dir,'perturbation_test.png'),bbox_inches = 'tight',dpi = 200)
# plt.show()
#%%

# com_data = sim_data.all_data[0].copy()[:,1,:].mean(axis = 0)
# time_vals = sim_data.all_time_vals
# plt.figure()
# plt.plot(time_vals[:np.where(time_vals == 20)[0][0]+1],np.abs(com_data - 0.45)[:np.where(time_vals == 20)[0][0]+1])
# plt.show()
...
# time_data = sim_data.all_time_vals.copy()
# norm_time = time_data/time_data.max()

# norm_time_plot_vals = np.round(np.arange(0,0.50,0.01),2)
# norm_time_plot_idx = np.searchsorted(norm_time, norm_time_plot_vals)
# # snapshot_time_vals = np.array([80,160,240,320,350])



# bright_palette = sns.color_palette("plasma",n_colors = norm_time_plot_idx.shape[0]+1)[::-1]
# ### Initial Filament Position
# fig,axes = plt.subplots(figsize = (8,8))
# plt.plot(adj_data[:,0,0],adj_data[:,1,0],color = bright_palette[0],label = 'Initial Position')
# for i,v in enumerate(norm_time_plot_idx):
#     t_val = v
#     axes.plot(adj_data[:,0,t_val],adj_data[:,1,t_val],color = bright_palette[i],label = r'$t/t_{{max}} = {0}$'.format(norm_time_plot_vals[i]))
