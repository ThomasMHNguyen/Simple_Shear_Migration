# -*- coding: utf-8 -*-
"""
FILE NAME:      A__v01_05_Brownian_sterics_Poiseuille.py
...
DESCRIPTION:    Given an analytical form for the rigidity of the filament and 
                ...
                specified duration. This script does not correct the filament velocity
                due to the sterics associated with 2 parallel walls. As such,
                sterics are ignored in this version of the script.
...
04Dec22         22) Code adjusted to run without sterics check.
06Feb23         23) If code fails, option to restart the entire code. 
...
STATUS:         Restart function sometimes work and sometimes doesn't.
...
import sys, os, math, time, random, argparse, logging, warnings, glob, re
...
class Constants:
    ...
    def __init__(self,end_dir,rigid_func,slender_f,mubar,true_zeta_f,adpt_zeta_f,top, bottom,U_centerline,
                 k_flow_phase,k_flow_factor,gap_criteria,force_power,
...
def eval_time_semi(sim_c,prev_params,curr_params):
    ...
    try:
        ...
        if sim_c.retry_type:
            ...
            logging.critical("Code has halted.")
            sys.exit(1)
...
def run_simulation(target_dir,rigid_type,mu_bar,rep):
    ...
    sim_c = Constants(end_dir = target_dir,rigid_func = rigid_type,
                                slender_f = 0.01,mubar = mu_bar,true_zeta_f =1e100,adpt_zeta_f = 6e7,
                                top = 0, bottom = 1,
                                U_centerline = 0,k_flow_phase = np.pi,k_flow_factor = 1,
                                gap_criteria = 0.02,force_power = 12,channel_height = 0.50,
                                vert_displace = 0.15,N = 101,lp = 1000,reg_dt = 1e-6,adpt_dt = 1e-8,
    ...
    while sim_c.curr_time < sim_c.end_time:
        ...
        if sim_c.record_it_count == 0:
            if np.isnan(sim_c.all_data[0][:,:,sim_c.ar_idx]).any():
                ...
                logging.warning(
                    "Error: Erroneous Filament Positional data detected.")
                logging.debug("Code will now stop at time = {:.8e} | iteration = {}".format(
                        sim_c.curr_time,sim_c.it_count))
                logging.debug("Random seed that generated the instability: {}".format(sim_c.rand_seed))
                    
                ### Save data to debugging CSV file ###
                failed_run_log_dir = os.path.join(output_dir,'failed_logs/')
                create_dir(failed_run_log_dir)
                save_fail_params = run_parameters(sim_c,"FAIL", "FAIL")
                failed_run_logs_all = glob.glob(os.path.join(failed_run_log_dir,'failed_run_log*.csv'))
                if failed_run_logs_all:
...
def Main():
    ...
    parser.add_argument("replicate_value",
                        ...
                        type = int)
    # args = parser.parse_args(['./01_Test_Results/Restart_Test/' ,'K_constant', '100000','1']) #Uncommment this line when debugging
