# -*- coding: utf-8 -*-
"""
FILE NAME:      A__v01_04_non_Brownian_sterics_Poiseuille.py
...
DESCRIPTION:    Given an analytical form for the rigidity of the filament and 
                ...
                due to the sterics associated with 2 parallel walls. This code
                is for non-Brownian simulations only.
...
19Sep22         22) Code reformatted for non-Brownian cases.
...
import sys, os, math, time, random, argparse, logging, warnings

### Specify to run using only one thread
...
def calculate_N_ex(sim_c,params):
    ...
    Txss,Tsxs,Kxs,Ksxs,Ksxsss,Kssxss = params
...
def steric_velocity_v2(sim_c,x_loc,non_elastic_U):
    
    
    upper_wall_coord = np.column_stack((x_loc[:,0],np.repeat(sim_c.channel_upper_height,sim_c.N),np.zeros(sim_c.N)))
    lower_wall_coord = np.column_stack((x_loc[:,0],np.repeat(-sim_c.channel_lower_height,sim_c.N),np.zeros(sim_c.N)))
    
    dist_upper_wall = x_loc - upper_wall_coord
    dist_lower_wall = x_loc - lower_wall_coord
        
    #Each of these should be a unit vector with 1 or -1 in the y-component; +1 for lower wall, -1 for upper wall
    p_hat_upper_wall = np.divide(dist_upper_wall,np.abs(dist_upper_wall),out = np.zeros_like(dist_upper_wall),where=np.abs(dist_upper_wall)!=0)
    p_hat_lower_wall = np.divide(dist_lower_wall,np.abs(dist_lower_wall),out = np.zeros_like(dist_lower_wall),where=np.abs(dist_lower_wall)!=0)
    
    U_N_upper_wall = rdot(p_hat_upper_wall,non_elastic_U)
    U_N_lower_wall = rdot(p_hat_lower_wall,non_elastic_U)
    ...
    U_normal_upper_wall = rdot(sim_c.phat_upper_wall,new_U)
    U_normal_lower_wall = rdot(sim_c.phat_lower_wall,new_U)
    
    #Tangential component of steric velocity is same as regular velocity because of enforcing slip velocity
    
    U_N_min_upper_wall = (1 - (delta/d_upper_wall[:,1])**m)*U_normal_upper_wall
    U_N_min_lower_wall = (1 - (delta/d_lower_wall[:,1])**m)*U_normal_lower_wall

    #Check if close to the wall
    high_wall_prox = np.where(np.abs(dist_upper_wall[:,1]) <=sim_c.gap_criteria)[0]
    low_wall_prox = np.where(np.abs(dist_lower_wall[:,1]) <=sim_c.gap_criteria)[0]
    if high_wall_prox.any() and not low_wall_prox.any():
        correct_U_higher = np.array([new_U[high_wall_prox,1],U_N_min_upper_wall[high_wall_prox]]).min(axis = 0) #Velocity should be in negative direction
    elif not high_wall_prox.any() and low_wall_prox.any():
...
class Constants:
    ...
    def __init__(self,end_dir,rigid_func,slender_f,mubar,true_zeta_f,adpt_zeta_f,top, bottom,U_centerline,gap_criteria,force_power,
    channel_height,vert_displace,N,reg_dt,adpt_dt,end_time,save_time):
        ...
        self.true_dt = reg_dt
        self.true_r1 = self.true_dt/(self.mu_bar*self.ds**4)
        self.true_r2 = self.true_dt/self.mu_bar
        
        #Parameters due to adaptive time setting
        self.adpt_dt = adpt_dt
        self.adpt_r1 = self.adpt_dt/(self.mu_bar*self.ds**4)
        self.adpt_r2 = self.adpt_dt/self.mu_bar
        ...
        #Parameters to use when calculating motion, tension#
        self.r1_use = 0
        self.r2_use = 0
        self.dt_use = 0
        self.zeta_use = 0
        ...
        ##### Determine orm of filament rigidity #####
        ...
        self.start_y = self.s*np.sin(self.theta) + self.vertical_displacement
        self.start_y += (1/200)*np.exp(self.s)**-4
        ...
        self.phat_upper_wall = np.column_stack((np.zeros(self.N),-1*np.ones(self.N),np.zeros(self.N)))
        self.phat_lower_wall = np.column_stack((np.zeros(self.N),1*np.ones(self.N),np.zeros(self.N)))
        ...
        self.all_data = [self.allstate,self.expl_U_states,self.all_u0_states,self.all_du0ds_states,self.all_dxds_states,self.all_d2xds2_states, #0-5
                    ...
                    self.all_Kxssss_states, self.all_Ksxs_states, self.all_Ksxsss_states, self.all_Kssxss_states, #11-14
                    self.all_forces_states, self.all_stress_states, #15-16
                    self.all_tension_states, self.all_dTds_states, #17-18
                    self.all_elastic_states, self.length,self.angle_calculations,self.deflection_calculations] #19-22
    ...
    def append_data(self,new_data):
        for i in range(0,17): #2D arrays
            self.all_data[i][:,:,self.ar_idx] = new_data[i]
        for i in range(17,19): #1D arrays
            self.all_data[i][:,self.ar_idx] = new_data[i]
        for i in range(19,23): #Singular values
            self.all_data[i][self.ar_idx] = new_data[i]
...
class run_parameters():
    def __init__(self,sim_params,end_time,start_time):
        self.parameter_df = pd.DataFrame(index = ['Random Seed','Filament Length','Filament Persistence Length','Filament s start','Filament s end',
                                              'Poiseuille U Centerline','Channel Upper Height','Channel Lower Height','Vertical Displacement',
        ...
        self.parameter_df.loc['Random Seed','Value'] ="None"
        self.parameter_df.loc['Filament Persistence Length','Value'] = "None"
        ...
        self.parameter_df.loc['Poiseuille U Centerline'] = 0
        ...
        self.parameter_df.loc['Array Time Step Size'] = sim_params.save_time
        self.parameter_df.loc['Number of Iterations needed for Calculation'] = sim_params.it_count
        self.parameter_df.loc['Simulation End Time'] = sim_params.end_time
        self.parameter_df.loc['Flow Type','Value'] = 'Shear'
        self.parameter_df.loc['Sterics Use','Value'] = 'Yes'
        self.parameter_df.loc['Brownian Use','Value'] = 'No'
...
def eval_time_semi(sim_c,prev_params,curr_params):
    ...
    try:
        ...
        save_fail_params.parameter_df.to_csv(os.path.join(sim_c.output_dir,'failed_run_log.csv'))
        
        ### Restart script ###
        # logging.critical("Code will now restart from the beginning. Pausing for 10 seconds before restart.")
        # time.sleep(10)
    ...
    ### Update time and determine whether to use adaptive or normal time step ###
    sim_c.update_time()
    ...
    var_to_pack = [future_xbar,corrected_U_N_ex,u0,du0ds,dxds, d2xds2, #0-5
                   ...
                   Kxssss,Ksxs,Ksxsss,Kssxss, #11-14
                   force,stress, #15-16
                   tension,dTds, #17-18
                   elastic_energy,fil_length,fil_angle,fil_deflect] #19-22
...
def eval_time_euler(sim_c):
    ...
    force_1 = calc_force(sim_c,Txss_1,Tsxs_1,Kssxss_1,Ksxsss_1,Kxssss_1,'all')
    ...
    Txss_1,Tsxs_1,Kxs_1,Ksxs_1,Ksxsss_1,Kssxss_1]
    ...
    fut_var_to_pack = [future_xbar,corrected_U_N_ex_1,u0_1,du0ds_1,dxds_1, d2xds2_1, #0-5
                   ...
                   Kxssss_1,Ksxs_1,Ksxsss_1,Kssxss_1, #11-14
                   force_1,stress_1, #15-16
                   tension_1,dTds_1, #17-18
                   elastic_energy_1,fil_length_1,fil_angle_1,fil_deflect_1] #19-22
...
def run_simulation(target_dir,rigid_type,mu_bar,rep):
    ...
    sim_c = Constants(end_dir = target_dir,rigid_func = rigid_type,
                                slender_f = 0.01,mubar = mu_bar,true_zeta_f =100*mu_bar,adpt_zeta_f = 1000*mu_bar,
                                top = 0, bottom = 100,U_centerline = 1.00,
                                gap_criteria = 0.02,force_power = 12,channel_height = 0.50,
                                vert_displace = 0.20,N = 101,reg_dt = 0.005,adpt_dt = 0.0005,
                                end_time = 40,save_time = 0.01)
    vert_displ_str = '{:.2f}'.format(sim_c.vertical_displacement).replace('.','p')
    U_val_str = '{:.2f}'.format(sim_c.U_centerline).replace('.','p')
    dir_name = 'Vert_Displ_{}/{}_U_{}/Mu_bar_{}/rep_{}'.format(vert_displ_str,
                                                                sim_c.rigidity_suffix,U_val_str,
                                                                int(sim_c.mu_bar),rep)
    # output_dir = os.path.join(sim_c.output_dir,dir_name)
    output_dir = sim_c.output_dir
    ...
    while sim_c.curr_time < sim_c.end_time:
        ...
        if sim_c.record_it_count == 0:
            if np.isnan(sim_c.all_data[0][:,:,sim_c.ar_idx]).any():
                ...
                ### Save data to debugging CSV file ###
                save_fail_params = run_parameters(sim_c, "FAIL", "FAIL")
                save_fail_params.parameter_df.to_csv(os.path.join(output_dir,'failed_run_log.csv'))
    ...
    np.save(os.path.join(output_dir,'filament_stress_all.npy'),sim_c.all_data[16])
    np.save(os.path.join(output_dir,'filament_tension.npy'),sim_c.all_data[17])
    np.save(os.path.join(output_dir,'filament_elastic_energy.npy'),sim_c.all_data[19])
    np.save(os.path.join(output_dir,'filament_length.npy'),sim_c.all_data[20])
    np.save(os.path.join(output_dir,'filament_angles.npy'),sim_c.all_data[21])
    np.save(os.path.join(output_dir,'filament_deflection.npy'),sim_c.all_data[22])
...
def Main():
    ...
    parser.add_argument("replicate_value",
                        ...
                        type = int)
    args = parser.parse_args(['./02_Actual_Results/Non_Brownian_Shear_Up/' ,'K_constant', '100000','1']) #Uncommment this line when debugging
...
adj_data = sim_data.all_data[0].copy()
adj_data[:,0,:] = adj_data[:,0,:] - adj_data[:,0,:].mean(axis = 0)

snapshot_time_vals = np.array([15,17.5,20,22.25,30])



bright_palette = sns.color_palette("bright",n_colors = snapshot_time_vals.shape[0]+1)
### Initial Filament Position
fig,axes = plt.subplots(figsize = (8,8))
plt.plot(adj_data[:,0,0],adj_data[:,1,0],color = bright_palette[0],label = 'Initial Position')
for i,v in enumerate(snapshot_time_vals):
    t_val = np.where(sim_data.all_time_vals == v)[0][0]
    plt.plot(adj_data[:,0,t_val],adj_data[:,1,t_val],color = bright_palette[i+1],label = 't = {}'.format(v))
...
# adj_s = -(1/200)*np.exp(s)**-4
# plt.figure(figsize = (7,7))
# plt.plot(s,adj_s,'r')
# plt.plot(s,-(1/200)*np.exp(-4*s),'b')
# plt.show()

#%% ####################### Post-Visualization: Center-of-mass Positions #######################

# adj_com_data = adj_data.mean(axis = 0).copy()
# t1_idx = np.where(sim_data.all_time_vals == 15)[0][0]
# t1p5_idx = np.where(sim_data.all_time_vals == 30)[0][0] 
# ### Initial Filament Position
# plt.figure(figsize = (8,8))
# plt.plot(adj_com_data[0,0],adj_com_data[1,0],'b',label = 'Initial COM',marker = 'x')
# plt.plot(adj_com_data[0,t1_idx],adj_com_data[1,t1_idx],'cyan',label = 't = {}'.format(15),marker = 'x')
# plt.plot(adj_com_data[0,t1p5_idx],adj_com_data[1,t1p5_idx],'magenta',label = 't = {}'.format(30),marker = 'x')
# plt.plot(adj_com_data[0,sim_data.ar_idx],adj_com_data[1,sim_data.ar_idx],'r',label = 'Final COM',marker = 'x')
# plt.axis('square')
...
matrix_number = np.arange(0,(3*N)**2, dtype=np.int64()).reshape(3*N,3*N)
indices = np.repeat(np.arange(0,3*N).reshape((N,3)), 3, axis=0)
diag_index = np.take_along_axis(matrix_number, indices, axis=1).flatten()
...
N = 101
s = np.linspace(-1/2,1/2,101)
theta = 99*np.pi/200
start_x = s*np.cos(theta)
start_y = s*np.sin(theta)
start_z = s*0
x_loc = np.column_stack((start_x,start_y,start_z))

upper_wall_coord = np.column_stack((x_loc[:,0],np.repeat(0.5,101),np.zeros(101)))
lower_wall_coord = np.column_stack((x_loc[:,0],np.repeat(-0.5,101),np.zeros(101)))
d_upper_wall = x_loc - upper_wall_coord
d_upper_wall2 = np.sqrt((x_loc[:,1] - 0.5)**2)
d_lower_wall = x_loc - lower_wall_coord


p_hat_upper_wall = np.divide(d_upper_wall,np.abs(d_upper_wall),out = np.zeros_like(d_upper_wall),where=np.abs(d_upper_wall)!=0)
p_hat_lower_wall = np.divide(d_lower_wall,np.abs(d_lower_wall),out = np.zeros_like(d_lower_wall),where=np.abs(d_lower_wall)!=0)

F_velocity = np.column_stack((np.random.rand(N),np.random.rand(N),np.zeros(N)))
F_velocity[:3,1] *= -1
F_velocity2 = F_velocity.copy()
F_N_upper_wall = rdot(p_hat_upper_wall,F_velocity)
F_N_lower_wall = rdot(p_hat_lower_wall,F_velocity)



delta = 0.024
m = 12
F_N_min_upper_wall = (1 - (delta/np.abs(d_upper_wall[:,1]))**m)*F_N_upper_wall
F_N_min_lower_wall = (1 - (delta/np.abs(d_lower_wall[:,1]))**m)*F_N_lower_wall

#Check if close to the wall
high_wall_prox = np.where(np.abs(d_upper_wall[:,1]) <=0.024)[0]
low_wall_prox = np.where(np.abs(d_lower_wall[:,1]) <=0.024)[0]
if high_wall_prox.any() and not low_wall_prox.any():
    correct_U_higher = np.array([F_velocity[high_wall_prox,1],F_N_min_upper_wall[high_wall_prox]]).min(axis = 0) #Velocity should be in negative direction
    F_velocity[high_wall_prox,1] = correct_U_higher
elif not high_wall_prox.any() and low_wall_prox.any():
    correct_U_higher = np.array([F_velocity[low_wall_prox,1],F_N_min_lower_wall[low_wall_prox]]).min(axis = 0) #Velocity should be in positive direction
    F_velocity[low_wall_prox,1] = correct_U_higher
elif high_wall_prox.any() and low_wall_prox.any():
    correct_U_higher = np.array([F_N_upper_wall[high_wall_prox],F_N_min_upper_wall[high_wall_prox]]).min(axis = 0) #Velocity should be in negative direction
    F_velocity[high_wall_prox,1] = correct_U_higher
    correct_U_higher = np.array([F_N_lower_wall[low_wall_prox],F_N_min_lower_wall[low_wall_prox]]).min(axis = 0) #Velocity should be in positive direction
    F_velocity[low_wall_prox,1] = correct_U_higher
