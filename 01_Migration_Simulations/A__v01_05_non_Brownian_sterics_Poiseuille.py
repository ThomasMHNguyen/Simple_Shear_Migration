...
def run_simulation(target_dir,rigid_type,mu_bar,rep):
    ...
    sim_c = Constants(end_dir = target_dir,rigid_func = rigid_type,
                                slender_f = 0.01,mubar = mu_bar,true_zeta_f =400*mu_bar,adpt_zeta_f = 1e4*mu_bar,
                                top = 0, bottom = 100,U_centerline = 1.00,
                                gap_criteria = 0.01,force_power = 20,channel_height = 0.50,
                                vert_displace = 0.25,N = 101,reg_dt = 0.001,adpt_dt = 0.00001,
...
def Main():
    ...
    parser.add_argument("replicate_value",
                        ...
                        type = int)
    args = parser.parse_args(['./02_Actual_Results/Non_Brownian_Poiseuille_Sterics_Test_1/' ,'K_constant', '20000','1']) #Uncommment this line when debugging
...
# snapshot_time_vals = np.array([2,4,8,12,40])
snapshot_time_vals = np.array([10,11.62,12,12.12])
...
axes.axhline(y = 0.5,xmin = 0,xmax = 1,color = 'black')
axes.set_xlim(-0.6,0.6)
axes.set_ylim(-0.6,0.6)

plt.show()

print(np.where(sim_data.all_data[0][:,1,:] > 0.5))
#%%
