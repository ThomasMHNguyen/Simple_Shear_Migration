...
class Constants:
    ...
    def __init__(self,end_dir,rigid_func,slender_f,mubar,true_zeta_f,adpt_zeta_f,
                 ...
                 gap_criteria,force_power,channel_height,vert_displace,N,
                 reg_dt,adpt_dt,end_time,save_time):
        ...
        self.start_y += (-1/200)*np.exp(-self.s)**4 #Downward flip
...
def run_simulation(target_dir,rigid_type,mu_bar,vert_displ,rep):
    """
    ...
    sim_c = Constants(end_dir = target_dir,rigid_func = rigid_type,
                                ...
                                top = 0, bottom = 200,U_centerline = 1.00,k_flow_phase = 0,k_flow_factor = 0,
                                gap_criteria = 0.02,force_power = 12,channel_height = 0.50,
                                vert_displace = vert_displ,N = 101,reg_dt = 0.005,adpt_dt = 0.0005,
                                end_time = 50,save_time = 0.01)
...
def Main():
    ...
    parser.add_argument("mu_bar",
                        ...
                        type = int)
    parser.add_argument("vertical_displacement",
                        help = "Specify the initial y-displacement value of the filament position to run the simulations on",
                        type = float)
    parser.add_argument("replicate_value",
                        help = "Specify what the replicate number will be in the numbering scheme",
                        type = int)
    args = parser.parse_args(['./02_Actual_Results/NB_Poiseuille_J_redo/UC_1p00_MB_5p00e4/Down_0p25/' ,'K_constant', '50000','0.25','1']) #Uncommment this line when debugging
    ...
    start_time = time.perf_counter()
    sim_c,save_sim_params = run_simulation(args.output_directory,
                                           args.rigidity_type,
                                           args.mu_bar,
                                           args.vertical_displacement,
                                           args.replicate_value)
    end_time = time.perf_counter()
