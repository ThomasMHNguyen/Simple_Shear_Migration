...
def center_of_mass(position_data,position,dim,adj_centering,adj_translation,transl_val):
    ...
    if dim == 3:
        if position == 0:
            if adj_centering:
                position_data[:,0,:] = position_data[:,0,:] - \
                    position_data[:,0,:].mean(axis = 0)#Adjust for any translation in x-coordinates
            if adj_translation:
                position_data[:,1,:] = position_data[:,1,:] - transl_val
                
            center_of_mass = position_data.mean(axis = position).copy().T #Rows are each time point, columns are x,y,z, components
